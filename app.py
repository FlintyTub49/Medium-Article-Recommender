...
def main():
    ...
    if choice == 'Recommendation System':
        ...
        if a:
            ...
            title = final.loc[indices[rand]]['Titles']
            rt = final.loc[indices[rand]]['Reading Time']
            link = final.loc[indices[rand]]['Story URL']
            actual = actual[rand]
        else:
    ...
    # ----------------------------------
        ...
        if search:
            ...
            if display.shape[0] > 0:
                ...
                # Checking if we have 15 values to print, otherwise we print the entire subset
                if display.shape[0] >= 15: 
                    samp = 15
                else:
                    samp = display.shape[0]
                
                midi.dataframe(display[['Titles', 'Max Prob Topic','Reading Time', 
